...
module Fosdem
  class Frab
    ...
    def initialize(hostaddress, port, user, password, database)
      ...
      log(:low, "connecting to Frab database //#{hostaddress}:#{port}/#{database} with user #{user}")
      begin
        @db = PGconn.open(
          host: hostaddress,
          port: port,
        ...
        log(:high, "connected to Frab database #{hostaddress}:#{port}", Time.now - time_before)
      rescue => e
        raise "Failed to connect to the Frab database //#{hostaddress}:#{port}/#{database} with user #{user}: #{e.message}"
      end
    end
    def close
      if @db
        @db.close
        log(:high, "disconnected from Frab database")
        @db = nil
      end
    end
    def update(config)
    ...
    def log(prio, message, duration=nil)
      ...
      Nanoc::CLI::Logger.instance.log(prio, "%s%12s%s  %s  %s" % [ "\e[36m", "frab", "\e[0m", t, message ])
    end
